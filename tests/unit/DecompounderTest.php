...
use paslandau\DictionaryDecompounder\DecompoundingPart;
...
class DecompounderTest extends PHPUnit_Framework_TestCase {
    ...
    public function test_ShouldRespondWithPredefinedDecompoundingResult(){
        ...
        $word = "Straßenschäden";
        $predefinedResult = new DecompoundingPart(
            $word,
            true,
            true
        );
    ...
    public function test_DecompoundingShouldWorkIfWordsAreInDict(){
        ...
        $allParts = $res->getDecompoundingParts();
        $parts = $allParts[0];
        /** @var DecompoundingPart $leftPart */
        $leftPart = $parts[0];
        ...
        $this->assertEquals(0, count($leftPart->getDecompoundingParts()), "The left part sohuld habe no futher decompounding parts");

        /** @var DecompoundingPart $rightPart */
        $rightPart = $parts[1];
        ...
        $this->assertEquals(0, count($rightPart->getDecompoundingParts()), "The right part sohuld habe no futher decompounding parts");
    }
    ...
    private function evaluateParts($parts, $expectedDecompoundings){
        ...
        $compare = [
            [$parts[0], $expectedParts["left"]],
            [$parts[1], $expectedParts["right"]]
        ];

        foreach($compare as $arr) {
            /** @var DecompoundingPart $actual */
            $actual = $arr[0];
            $expected = $arr[1];
            ...
            if (array_key_exists("children", $expected)) {
                $allParts = $actual->getDecompoundingParts();
                foreach($allParts as $key => $innerParts){
